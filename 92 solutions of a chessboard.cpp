#Project by Ali Ashrar
#A fancy chessboard where by running the code, prints out 92 solutions of a game with the queen displayed on the board.
      ...
      for (k=0; k<5; k++) {     // for each box row
         ...
         for (j=0; j<8; j++)    // for each board column
            for (l=0; l<7; l++) // for each box column
               ...
               cout << (*board[i][j])[k][l]; //*board[i][j] is the box itself. Box is a 2d array, so the whole thing is the element in the box at row k col l.
         cout << char(179) << "\n";
      }


      //board[i][j] is a pointer to a box.
      //*board[i][j] is the box itself
